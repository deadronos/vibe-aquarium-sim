...
3.  **Physics -> ECS**: Inside `Fish.tsx`'s `useFrame`, we read the *actual* state from the `RigidBody` (which accounts for collisions) and write it back to the ECS entity.
